...
- Relative labels are only used for jumps, and jumps only use relative labels
- Absolute labels refer either to code or to initialised (pre-populated) arrays
- No explicit use of `LIT`, so only the `#` syntax
- You can skip `INC`, `SFT`, `EOR`, `AND`, `ORA`, `DEI`, and the stack operations `ROT`, `OVR`, `SWP`, `POP`
- No parentheses inside comments

### Python reference implementation and Uxntal test cases

I provide the starting point code for the interpreter this exercise, as well as a set of 16 test cases. Toghether, this serves as the functional specification. 
...
* [5 marks] Your interpreter correctly runs a non-trivial program of your choice
